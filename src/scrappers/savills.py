...
def parse_property(auction_id, lot_id, auction_date, venue):
    proccess_lot_url = (
        "https://auctions.savills.co.uk/index.php?option=com_bidding&format=json&task=commission.processLot"
    )
    processlot_payload = {"auction_id": auction_id, "lot_id": lot_id}
    ...
    description = (
        json_data["lot"]["description"]
        + "\n"
        + json_data["lot"]["key_features"]
        + "\n"
        + json_data["lot"]["accommodation"]
    )
