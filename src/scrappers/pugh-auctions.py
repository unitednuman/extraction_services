...
def parse_property(auction_url, auction_image, auction_title, auction_price, property_type):
    try:
        ...
        try:
            auction_date = (
                result.xpath(
                    "//span[@class='inline-block vertical-middle line-height-1pt4 font-size-18 font-size-24-print font-weight-600 colour-blue margin-b5']"
                )[1]
                .text_content()
                .replace("- Bidding opens", "")
                .strip()
            )
...
def _run(url, property_type):
    ...
    try:
        result = results.xpath("//div[@class='property-listing padding-20 ']") + results.xpath(
            "//div[@class='property-listing padding-20 even']"
        )
        ...
        for auction in result:
            try:
                ...
                auction_price = (
                    auction.xpath(".//div[@class='block font-size-26 font-weight-600']")[0].text_content().strip()
                )
...
def run():
    property_types = [
        "Block%20of%20Apartments",
        "Bungalow",
        "Caravan%2FPark%20Home",
        "Commercial%20Property",
        "Detached",
        "Farm%20House%2FBarn%20Conversion",
        "Flat%2FApartment",
        "Hotel%2FGuesthouse",
        "Land",
        "Other",
        "Parking%2FGarages",
        "Public%20Convenience",
        "Retirement%20Property",
        "Semi%20Detached",
        "Terraced",
    ]
