...
from extraction_services.models import LoggerModel, disable_other_loggers
...
def get_text(nodes, index, xpath):
    try:
        ...
        try:
            ...
            LoggerModel.debug(f"error while fetching filenames: {ex}")
            filenames = ""
        save_error_report(e, filenames, secondary_error=True)
        LoggerModel.debug(f"could not find text with Xpath = {xpath}, with exception {e}")
        return None
