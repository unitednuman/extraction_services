...
def parse_property(page, url,price, currency,auction_img,auction_datetime_new):
    ...
    description = result.xpath("//div[@class='twelve fluid columns']")[0].text_content().strip().replace("< Back to Lot ListNext Lot >Lot\n","").replace("\n< Back to Lot ListNext Lot >","")
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            auction_datetime_new = datetime(
                auction_time.year, auction_time.day, auction_time.month, hour=10
            )
            page.goto(url)
            time.sleep(3)
            results = html.fromstring(page.content())
            fix_br_tag_issue(results)
            for auction in results.xpath("//div[@style='margin-bottom:20px; background:#fafafa; border:solid 1px #efefef; padding:15px;']"):
                guide_price=auction.xpath(".//p[@class='price']")[0].text_content().strip()
                price, currency = prepare_price(guide_price)
                auction_url="https://www.networkauctions.co.uk/online-auction-catalogues/"+auction.xpath(".//a[@class='button']")[0].attrib["href"]
